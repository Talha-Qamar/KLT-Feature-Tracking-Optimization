...
#ifdef USE_OPENACC
    {
      const int startX = borderx;
      const int endX = ncols - borderx;
      const int startY = bordery;
      const int endY = nrows - bordery;
      const int nx = (endX > startX) ? ((endX - startX + stride - 1) / stride) : 0;
      const int ny = (endY > startY) ? ((endY - startY + stride - 1) / stride) : 0;
      const int totalCandidates = nx * ny;
      const int totalPixels = ncols * nrows;
      const int window_w = 2 * window_hw + 1;
      const float *gradx_data = gradx->data;
      const float *grady_data = grady->data;

      npoints = totalCandidates;
      if (totalCandidates > 0) {
#pragma acc data copyin(gradx_data[0:totalPixels], grady_data[0:totalPixels]) copyout(pointlist[0:totalCandidates*3])
        {
#pragma acc parallel loop gang collapse(2) vector_length(128)
          for (int yi = 0 ; yi < ny ; yi++) {
            for (int xi = 0 ; xi < nx ; xi++) {
              const int x = startX + xi * stride;
              const int y = startY + yi * stride;
              float gxx = 0.0f, gxy = 0.0f, gyy = 0.0f;
#pragma acc loop seq
              for (int wy = -window_hh ; wy <= window_hh ; wy++) {
                const int row = y + wy;
                const int row_base = row * ncols + (x - window_hw);
#pragma acc cache(gradx_data[row_base:window_w], grady_data[row_base:window_w])
#pragma acc loop vector reduction(+:gxx,gxy,gyy)
                for (int wx = 0 ; wx < window_w ; wx++)  {
                  const int idx = row_base + wx;
                  const float gx = gradx_data[idx];
                  const float gy = grady_data[idx];
                  gxx += gx * gx;
                  gxy += gx * gy;
                  gyy += gy * gy;
                }
              ...
              const int base = (yi * nx + xi) * 3;
              pointlist[base]     = x;
              pointlist[base + 1] = y;
              pointlist[base + 2] = (int) val;
            }
          }
        }
      }
...
#else
    {
      ...
      ptr = pointlist;
      for (int y = bordery ; y < nrows - bordery ; y += stride) {
        for (int x = borderx ; x < ncols - borderx ; x += stride)  {
          ...
          if (val > limit)  val = (float) limit;
          /* Serialize writes back to pointlist */
          *ptr++ = x;
          *ptr++ = y;
          *ptr++ = (int) val;
          npoints++;
  ...
  KLT_FeatureList fl)
{
  ...
  fprintf(stderr, "[CPU] KLTSelectGoodFeatures: %.3f ms\n", (__t1 - __t0) / 1000.0);
#endif
}
