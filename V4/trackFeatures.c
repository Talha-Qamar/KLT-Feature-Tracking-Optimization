  ...
  _KLT_FloatImage img)
{
  ...
  const int stride = img->ncols;
  const float p00 = ptr[0];
  const float p10 = ptr[1];
  const float p01 = ptr[stride];
  const float p11 = ptr[stride + 1];
...
static _FloatWindow _ensureWorkspace(int width, int height, _FloatWindow *buffer, int *bufWidth, int *bufHeight)
{
  if (*buffer == NULL || width > *bufWidth || height > *bufHeight) {
    if (*buffer != NULL) free(*buffer);
    *buffer = _allocateFloatWindow(width, height);
    *bufWidth = width;
    *bufHeight = height;
  }
  return *buffer;
...
{
  static _FloatWindow imgdiff_buf = NULL;
  static _FloatWindow gradx_buf = NULL;
  static _FloatWindow grady_buf = NULL;
  static int imgdiff_w = 0, imgdiff_h = 0;
  static int gradx_w = 0, gradx_h = 0;
  static int grady_w = 0, grady_h = 0;
  ...
  const int window_area = width * height;

	
  /* Acquire reusable workspace windows */
  imgdiff = _ensureWorkspace(width, height, &imgdiff_buf, &imgdiff_w, &imgdiff_h);
  gradx   = _ensureWorkspace(width, height, &gradx_buf, &gradx_w, &gradx_h);
  grady   = _ensureWorkspace(width, height, &grady_buf, &grady_w, &grady_h);
...
#ifdef USE_OPENACC
    gxx = 0.0f; gxy = 0.0f; gyy = 0.0f; ex = 0.0f; ey = 0.0f;
#pragma acc data copyin(gradx[0:window_area], grady[0:window_area], imgdiff[0:window_area])
    {
#pragma acc parallel loop reduction(+:gxx,gxy,gyy,ex,ey)
      for (idx = 0; idx < window_area; ++idx) {
        const float gx = gradx[idx];
        const float gy = grady[idx];
        const float diff = imgdiff[idx];
        gxx += gx * gx;
        gxy += gx * gy;
        gyy += gy * gy;
        ex += diff * gx;
